...
DEFAULT_AGENT_NAME = os.getenv(
    "DEFAULT_AGENT_NAME", meta.get("name", "Ansible Tower Mcp")
)
DEFAULT_AGENT_DESCRIPTION = os.getenv(
    "AGENT_DESCRIPTION",
    meta.get("description", "AI agent for Ansible Tower management."),
)
...
def agent_template(mcp_url: str = None, mcp_config: str = None, **kwargs):
    ...
    from ansible_tower_mcp.graph_config import TAG_PROMPTS, TAG_ENV_VARS
        ...
        try:
            ...
            if not os.path.isabs(config_path) and "/" not in config_path:
                ...
                try:
                    ...
                    pkg_res = files("ansible_tower_mcp") / config_path
                    if pkg_res.is_file():
            ...
            if os.path.exists(config_path):
                ...
                logger.info(
                    f"ansible-tower-mcp: Loaded {len(mcp_toolsets)} MCP servers from {config_path}"
                )
        except Exception as e:
            logger.error(
                f"ansible-tower-mcp: Failed to load MCP config {effective_mcp_config}: {e}"
            )
