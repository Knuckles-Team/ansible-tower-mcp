# ansible-tower-mcp — Concept Overview
...
```json
{
  "mcpServers": {
    "ansible-tower-mcp": {
      "command": "uv",
      "args": ["run", "--with", "ansible-tower-mcp", "ansible-mcp"],
      "env": {}
    }
  }
}
...
```bash
ansible-mcp --transport streamable-http --port 8001
```
